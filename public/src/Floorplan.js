...
export class Floorplan extends go.Diagram {
  ...
  constructor(div) {
    ...
    this.toolManager.draggingTool.doDeactivate = function () {
      ...
      this.isGridSnapEnabled =
        this.diagram.model.modelData.preferences.gridSnap;
        ...
        if (
          ...
          if (resizeAdornment !== null) {
            ...
            while (ritr.next()) {
              ...
              if (handle !== null) {
                if (
                  el instanceof go.Shape &&
                  el.alignment === handle.alignment
                ) {
          ...
          wall.memberParts.iterator.each(function (part) {
            if (part.data.key !== obj.data.key) {
              ...
              for (let i = 0; i < endpoints.length; i++) {
                ...
                const distanceToMovingPt = Math.sqrt(
                  point.distanceSquaredPoint(movingPt)
                );
          ...
          if (stationaryPt !== null) {
            maxLength = Math.sqrt(
              stationaryPt.distanceSquaredPoint(constrainingPt)
            );
          }
  ...
  checkboxChanged(checkboxId) {
    ...
    switch (checkboxId) {
      ...
      case "wallGuidelinesCheckbox":
        floorplan.model.modelData.preferences.showWallGuidelines =
          element.checked;
        break;
      ...
      case "onlySmallWallAnglesCheckbox": {
        floorplan.model.modelData.preferences.showOnlySmallWallAngles =
          element.checked;
        floorplan.updateWallAngles();
        break;
      }
  ...
  changeUnits(form) {
    ...
    const oldUnitsConversionFactor = parseFloat(
      unitsConversionFactorInput.value
    );
    const units = fp.model.modelData.units;
    const newUnitsConverstionFactor = fp.convertUnits(
      prevUnits,
      units,
  ...
  getRoomWalls(pt) {
    ...
    walls.iterator.each(function (w) {
      const ip = fp.getSegmentsIntersection(
        pt,
        oPt,
        w.data.startpoint,
        w.data.endpoint
      ...
      ccWalls.iterator.each(function (w) {
        ...
        if (
          ...
          if (path === undefined || path === null) {
            ...
            const ip1 = fp.getLinesIntersection(
              pt,
              oPt,
              wall.data.smpt1,
              wall.data.empt1
            );
            const ip2 = fp.getLinesIntersection(
              pt,
              oPt,
              wall.data.smpt2,
              wall.data.empt2
    ...
    for (let i = 0; i < wallsDistArr.length; i++) {
      ...
      possiblePaths = recursivelyFindPaths(
        w,
        null,
        possiblePaths,
        null,
    ...
    if (roomOuterBoundaryPts !== null) {
      ...
      const newRoomBoundaryWalls = fp.addInternalWallsToRoom(
  ...
  addInternalWallsToRoom(roomOuterBoundaryPts, roomOuterBoundaryPath) {
    ...
    function recursivelyFindInteralClusterPath(
      ...
      iwalls.iterator.each(function (fw) {
        ...
        if (fw.data.key !== wall.data.key) {
          ...
          path.push(entry);
          const otherEndpoint = wrt.pointsApproximatelyEqual(
            ip,
            fw.data.startpoint
          )
            ? fw.data.endpoint
          ...
          if (iw.count > 1 && doContinue) {
            return recursivelyFindInteralClusterPath(
              fw,
              iw,
              otherEndpoint,
              seenIp,
    ...
    offendingWalls.iterator.each(function (ow) {
      if (!handledWalls.contains(ow)) {
        ...
        if (
          ...
          roomOuterBoundaryPath[roomOuterBoundaryPath.length - 1][0] ===
            bw2.data.key
        ) {
          let temp = null;
          temp = bw1;
        ...
        if (ip !== null) {
          ...
          // sort intersectionWalls s.t. they are clockwise, starting with bw1
          if (bw1 !== null) {
            intersectionWalls = fp.sortWallsClockwiseWithSetStartWall(
              intersectionWalls,
              bw1
            );
          }
    ...
    internalPathsToInsert.iterator.each(function (kvp) {
      ...
      const secondArr = roomOuterBoundaryPath.slice(
        insertionIndex,
        roomOuterBoundaryPath.length
      );
  ...
  findRoomHoles(roomBoundaryWalls, pt) {
    ...
    function recursivelyFindHolePath(w, seenIp, path, prevPt, origWall) {
      ...
      // have we come full circle?
      const isNextClockwiseWallOrigWall =
        iw.count > 1 && iw.toArray()[1].data.key === origWall.data.key;
      const atFinalIp =
        (isNextClockwiseWallOrigWall &&
          (w.data.key !== origWall.data.key || isSoloWallHole)) ||
      ...
      if (doContinue) {
        ...
        iw.iterator.each(function (fw) {
          ...
          if (
            path !== null &&
            fw.data.key !== w.data.key &&
            path.indexOf("isDone") === -1
          ) {
  ...
  isPointInRoom(r, p) {
    ...
    if (
      r === null ||
      r === undefined ||
      !(r instanceof go.Node) ||
      r.data.category !== "RoomNode"
    )
      return false;
    ...
    if (wall.data.startpoint && wall.data.endpoint) {
      ...
      geo = new go.Geometry().add(
        new go.PathFigure(data.startpoint.x, data.startpoint.y)
          .add(
            new go.PathSegment(go.SegmentType.Line, data.smpt1.x, data.smpt1.y)
          )
          .add(
            new go.PathSegment(go.SegmentType.Line, data.empt1.x, data.empt1.y)
          )
          .add(
            new go.PathSegment(
              go.SegmentType.Line,
          ...
          .add(
            new go.PathSegment(go.SegmentType.Line, data.empt2.x, data.empt2.y)
          )
          .add(
            new go.PathSegment(go.SegmentType.Line, data.smpt2.x, data.smpt2.y)
          )
          .add(
            new go.PathSegment(
              go.SegmentType.Line,
  ...
  updateRoom(room) {
    ...
    function addPathToGeo(boundaryWalls) {
      ...
      if (fig === null) {
        ...
        fig.add(
          new go.PathSegment(go.SegmentType.Move, farthestPt.x, farthestPt.y)
        );
      }
      fig.add(
        new go.PathSegment(go.SegmentType.Line, closestPt.x, closestPt.y)
      );
      let prevPt = closestPt;
      let prevWall = firstWall;
      for (let i = 0; i < boundaryWalls.length; i++) {
        ...
        if (i !== 0) {
          ...
          if (prevWall.data.key === w.data.key) {
            fig.add(
              new go.PathSegment(go.SegmentType.Line, prevPt.x, prevPt.y)
            );
            fig.add(
              new go.PathSegment(
                go.SegmentType.Line,
          ...
          fig.add(
            new go.PathSegment(go.SegmentType.Line, closestPt1.x, closestPt1.y)
          );
          fig.add(
            new go.PathSegment(
              go.SegmentType.Line,
    ...
    fp.model.setDataProperty(
      room.data,
      "loc",
      new go.Point(smallestX, smallestY)
    );
  ...
  buildDimensionLink(
    ...
    link.data.length = Math.sqrt(
      pointNode1.location.distanceSquaredPoint(pointNode2.location)
    );
  }
  /**
      ...
      for (let i = 1; i < 3; i++) {
        ...
        wall.memberParts.iterator.each(function (wallPart) {
          if (wallPart.isSelected) {
            ...
            const newEp1 = floorplan.rotateAndTranslatePoint(
              ep1,
              angle + 0,
              wall.data.thickness / 2
            );
            const newEp2 = floorplan.rotateAndTranslatePoint(
              ep2,
              angle + 0,
              wall.data.thickness / 2
            );
        ...
        for (let j = 0; j < wallPartEndpoints.length - 1; j++) {
          ...
          if (linkPoint1 !== null && linkPoint2 !== null) {
            const newLoc1 = floorplan.getAdjustedPoint(
              wallPartEndpoints[j].copy(),
              wall,
              angle,
              15
            );
            const newLoc2 = floorplan.getAdjustedPoint(
      ...
      for (let i = 0; i < selectedWalls.length; i++) {
        ...
        possibleWalls.iterator.each(function (otherWall) {
          ...
          if (
            ...
            const intersectionPoint = floorplan.getWallsIntersection(
              wall,
              otherWall
            );
            if (intersectionPoint !== null) {
              ...
              wallsInvolved.iterator.each(function (w) {
                ...
                if (
                  Math.sqrt(
                    w.data.startpoint.distanceSquaredPoint(intersectionPoint)
                  ) > tolerance
                )
                  endpoints.push({
                    point: w.data.startpoint,
                    wall: w.data.key,
                  });
                if (
                  Math.sqrt(
                    w.data.endpoint.distanceSquaredPoint(intersectionPoint)
                  ) > tolerance
                )
              ...
              endpoints.sort(function (a, b) {
                ...
                b = b.point;
                if (
                  a.x - intersectionPoint.x >= 0 &&
                  b.x - intersectionPoint.x < 0
                )
                  return 1;
                if (
                  a.x - intersectionPoint.x < 0 &&
                ...
                if (
                  ...
                  if (
                    a.y - intersectionPoint.y >= 0 ||
                    b.y - intersectionPoint.y >= 0
                  )
                    return a.y > b.y ? 1 : -1;
                  return b.y > a.y ? 1 : -1;
                }
  ...
  findClosestLocOnWall(wall, part) {
    ...
    for (let i = 0; i < possibleStretches.length; i++) {
      ...
      const testDistance1 = Math.sqrt(
        testPoint1.distanceSquaredPoint(part.location)
      );
      const testDistance2 = Math.sqrt(
        testPoint2.distanceSquaredPoint(part.location)
      );
    ...
    const pt1 = new go.Point(
      closestStretch.pt1.x +
        (offset / closestStretchLength) *
          (closestStretch.pt2.x - closestStretch.pt1.x),
      closestStretch.pt1.y +
        (offset / closestStretchLength) *
          (closestStretch.pt2.y - closestStretch.pt1.y)
    );
    const pt2 = new go.Point(
      closestStretch.pt2.x +
        (offset / closestStretchLength) *
          (closestStretch.pt1.x - closestStretch.pt2.x),
      closestStretch.pt2.y +
        (offset / closestStretchLength) *
          (closestStretch.pt1.y - closestStretch.pt2.y)
    );
  ...
  return $(
    ...
    $(
      ...
      new go.Binding("text", "", function (text, obj) {
        ...
        const name =
          obj.part.adornedObject.category === "MultiPurposeNode"
            ? data.text
            : data.caption;
        return (
          "Name: " +
  ...
  return $(
    ...
    $(
      ...
      new go.Binding("stroke", "isSelected", function (s, obj) {
        return s
          ? go.Brush.lightenBy(obj.stroke, 0.5)
          : invertColor(obj.part.data.color);
      }).ofObject()
    ),
    $(
      go.Panel,
...
function updateNodeTexture(obj, t) {
  ...
  if (node.data.usesTexture !== undefined && node.data.usesTexture) {
    if (t === "") {
      ...
      const str =
        t.indexOf("images/textures") === -1 ? "images/textures/" + t : t;
  ...
  if (
    ...
    if (newLoc !== null) {
      ...
      if (wallPart.category === "WindowNode") {
        floorplan.model.setDataProperty(
          wallPart.data,
          "height",
          wall.data.thickness
        );
  ...
  return $(
    ...
    "Spot",
    {
      ...
      dragComputation: function (part, pt, gridPt) {
        ...
        const wallsAtStartpoint = wrt.getAllWallsAtIntersection(
          part.data.startpoint,
          true
        );
        ...
        const wallsAtEndpoint = wrt.getAllWallsAtIntersection(
          part.data.endpoint,
          true
        );
        wallsAtEndpoint.iterator.each(function (w) {
          if (w.data.key !== part.data.key) {
            if (
              ...
              wrt.pointsApproximatelyEqual(w.data.endpoint, part.data.endpoint)
            ) {
              connectedWallsMap.add(w.data.key, {
                connectedTo: "endpoint",
                connectedFrom: "endpoint",
        ...
        const ptToUse = fp.toolManager.draggingTool.isGridSnapEnabled
          ? gridPt
          : pt;
        const wall = part;
        ...
        function moveAndUpdate(pointToUse) {
          ...
          fp.model.set(
            ...
            "startpoint",
            new go.Point(
              part.data.startpoint.x + dx,
              part.data.startpoint.y + dy
            )
          );
          fp.model.set(
            part.data,
        ...
        allWalls.iterator.each(function (n) {
          ...
          if (
            wall &&
            wall.data.key !== w.data.key &&
            fp.getWallsIntersection(wall, w)
          ) {
    ...
    $(
      ...
      $(
        ...
        $(
          ...
          new go.Binding("figure", "showTextureOptions", function (sto) {
        ...
        "Horizontal",
        {
          ...
          itemTemplate: $(
            ...
            $(
              go.Picture,
              { name: "BUTTON_IMAGE" },
              new go.Binding("source", "", function (str, b) {
    ...
    $(
        ...
        $(
          ...
          $(
            ...
            new go.Binding("text", "", function (room) {
              ...
              return (
                adjustedArea +
                fp.model.modelData.unitsAbbreviation +
      ...
      $(
        ...
        $(
          ...
          new go.Binding("figure", "showFlooringOptions", function (
            showFlooringOptions
          ) {
...
function getWallPartStretch(part) {
  ...
  if (
    parseFloat(startpoint.x.toFixed(2)) <
      parseFloat(part.location.x.toFixed(2)) ||
  ...
  if (
    parseFloat(endpoint.x.toFixed(2)) <
      parseFloat(part.location.x.toFixed(2)) ||
...
const dragWallParts = function (part, pt, gridPt) {
  if (
    part.containingGroup !== null &&
    part.containingGroup.category === "WallGroup"
  ) {
    ...
    if (dist1 + dist2 !== totalDist) {
      part.location = part.location
        .copy()
        .projectOntoLineSegmentPoint(wStart, wEnd);
    }
    // main behavior
    ...
    const totalLength = Math.sqrt(
      leftOrAbovePt.distanceSquaredPoint(rightOrBelowPt)
    );
    const distance = part.data.length / 2;
    const point1 = new go.Point(
      leftOrAbovePt.x +
        (distance / totalLength) * (rightOrBelowPt.x - leftOrAbovePt.x),
      leftOrAbovePt.y +
        (distance / totalLength) * (rightOrBelowPt.y - leftOrAbovePt.y)
    );
    const point2 = new go.Point(
      rightOrBelowPt.x +
        (distance / totalLength) * (leftOrAbovePt.x - rightOrBelowPt.x),
      rightOrBelowPt.y +
        (distance / totalLength) * (leftOrAbovePt.y - rightOrBelowPt.y)
    );
    ...
    const distFromWall =
      ...
      Math.sqrt(
        Math.pow(wEnd.y - wStart.y, 2) + Math.pow(wEnd.x - wStart.x, 2)
      );
    const tolerance =
      20 * wall.data.thickness < 100 ? 20 * wall.data.thickness : 100;
    ...
    $(
      ...
      $(
        ...
        new go.Binding("visible", "", function (obj) {
          return obj.adornedPart === null
            ? false
            : obj.adornedPart.containingGroup !== null;
        }).ofObject()
      ),
      $(
        "Button",
